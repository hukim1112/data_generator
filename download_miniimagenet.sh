...
user=krokerry
DATADIR=datasets/mini-imagenet
...
do
   wget -c --output-document=$DATADIR/train/$i.tar "http://www.image-net.org/download/synset?wnid=$i&username=$user&accesskey=$token&release=latest&src=stanford"
   mkdir $DATADIR/train/$i
   tar xf $DATADIR/train/$i.tar -C $DATADIR/data/train/$i/
done

# Download test set
for j in "${testclasses[@]}"
do
   wget -c --output-document=$DATADIR/test/$j.tar "http://www.image-net.org/download/synset?wnid=$j&username=$user&accesskey=$token&release=latest&src=stanford"
   mkdir $DATADIR/test/$j
   tar xf $DATADIR/test/$j.tar -C $DATADIR/test/$j/
done


python create_miniimagenet.py